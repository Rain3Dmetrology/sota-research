...
| 维度 | 数据源 / Skill | 用途 | 状态 |
|------|--------|------|------|
| **搜索入口** | 内置 WebSearch/WebFetch + **Tavily**（API key） | 通用联网检索、查证 | ✅ 真实可用 |
| **AI 搜索（可选）** | **Perplexity**（需 `PERPLEXITY_API_KEY`） | 带引用的 AI 搜索，与 Tavily 并列可选，无 key 跳过 | 可选源（需 key） |
| **社媒 / 热评** | **agent-reach** / **agent-browser** / web-access | 小红书/知乎/Reddit/Bluesky/X/评论抓取 | ✅ 真实可用 |
...
| **代码 / 项目** | **GitHub**（连接器已连） | 开源实现、技术栈、Star/PR 趋势 | ✅ 真实可用 |
| **专利** | 智慧芽 / PatSnap | 技术壁垒、专利家族、引用分析 | 按需连接 |
...
| **学术 / 论文（扩展）** | arXiv / PubMed / Semantic Scholar / Google Scholar / CNKI（**academic-research-hub** / **literature-search** 提供多源检索 + 引文处理） | 学术论文、引用网络、BibTeX/RIS/JSON 引文 | 🌐 通用联网可达 |
...
| **代码 / 模型平台** | Hugging Face / 魔塔 ModelScope | AI 模型、代码、应用文档、数据集 | 🌐 通用联网可达（ModelScope 如有 API key 可直接调用） |
| **开放百科** | Wikipedia / 百度百科 | 概念科普、背景知识 | 🌐 通用联网可达 |
| **产品 / 创投** | Product Hunt / TechCrunch / 36氪 / 虎嗅 | 新品发布、融资、市场热度 | 🌐 通用联网可达 |
| **开发者社区** | Hacker News / Stack Overflow / Reddit / GitHub Discussions | 开发者真实反馈、技术选型讨论 | 🌐 通用联网可达 |
| **中文社区** | 知乎 / CSDN / 博客园 / V2EX / 小红书 / B站 | 用户反馈、产品体验、教程 | 🌐 通用联网可达 |
...
- 🌐 **通用联网可达**：无专用 Skill/连接器，通过 WebSearch/WebFetch/agent-reach/agent-browser 抓取；按源分级处理（通常 T3 媒体/官方记录或 T4 UGC 信号）。
- **可选源（免 key 或公开）**：公开可访问的新闻/资讯站点，无需专用连接器。

> **诚实声明（重要）**：本 Skill 只集成**当前环境真实存在**的 Skill / 连接器。Firecrawl、Crunchbase Pro、PitchBook 等在本环境未提供的服务**不会**被声明为已集成——若你所在平台提供，可自行在 Step 1 搜索入口追加，但 README / SKILL.md 不虚假标注。Tavily、GitHub、agent-reach、agent-browser、markitdown、tdx-connector、perplexity、wechat-article-search、academic-research-hub、literature-search、news-summary、intel-osint-daily、macro-monitor、deep-research 等为当前环境已安装的互补 skill，dmr v2.1.0 在「互补 Skill 方法论吸收」小节中按需作为可选源/方法论引用（均优雅降级）。

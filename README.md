...
> The core difference from generic AI search / deep-research skills: **dmr is not a search wrapper, but a reproducible, confidence-labeled, adversarially audited research pipeline.**

### Version Evolution (latest first)

- **v2.2.7 P1 Integration + Curation**: structured markdown asset accumulation + optional hyperresearch deep backend + Step 1 intent routing + native Chinese/CJK support.
- **v2.2.6 Adversarial Audit Discipline**: corpus critic + 4 parallel critic roles + patch-never-regenerate + provenance source tree + pre-delivery lint checklist.
- **v2.2.5 Search Methodology Sharpening**: information-density priority, source-diversity weighting, semantic × recency × source-tier tri-axis ranking.
- **v2.2.4 Normative Enhancement**: FAQ, end-to-end example, full changelog appendix.

### Unique Advantages

- **Deterministic pipeline**: fixed Step 0–8, reproducible and comparable across runs.
- **Confidence-labeled source tiers**: T1 official / T2 expert / T3 secondary / T4 social.
- **≥2-source cross-validation**: fact-unit decomposition, conflicts explicitly flagged, no forced consensus.
- **Adversarial pre-delivery audit**: independent critic pass, surgical patch only, no full rewrite.
- **Native Chinese/CJK support**: WeChat, Zhihu, Xiaohongshu, CNKI treated as first-class sources, not junk.
- **Zero-install Skill**: pure methodology, uses agent built-in tools, no extra Python dependency.
- **Optional backends never block**: Tavily / Perplexity / hyperresearch enhance when available, gracefully degrade otherwise.

### Output Capabilities

- **Three templates**: general research / industry track (McKinsey-style) / company competitive analysis (SWOT + scenario planning).
- **intel-brief style**: fact → impact → cause triad.
- **Academic module**: arXiv / PubMed / OpenAlex / Semantic Scholar / CNKI, prioritizing 🆓 free APIs.
- **Analysis lenses**: Porter's Five Forces / PESTEL / BCG / 3C / TAM-SOM, triggered by intent, no bloat.
- **Incremental accumulation**: structured markdown notes with YAML frontmatter, wired to ima / Obsidian / local wiki.
...
| 维度 | 数据源 / Skill | 用途 | 状态 |
|------|--------|------|------|
| **搜索入口** | 内置 WebSearch/WebFetch（优先）+ **Tavily**（API key 直调，skill 已于 v2.2.1 永久删除）+ **AgentKey**（聚合数据 API：搜索/新闻/社媒/股票/企业/实时，已连） | 通用联网检索、查证、聚合数据 | ✅ 真实可用（AgentKey 已连） |
...
| **法律 / 合规** | 威科先行 / 元典 / **北大法宝（pkulaw，已连未启用）** | 诉讼、资质、行政处罚、法律法规检索 | 按需连接 |
| **企业工商 / 风险** | 天眼查 MCP / 企查查 MCP / **启信慧眼（qixinhuiyan，按需）** | 股权、司法、经营异常、知识产权、企业风险洞察 | 按需连接（当前未启用） |
...
| **知识库** | ima-mcp / Obsidian / 本地 wiki / **notion（已连）** | 用户自有资料、增量 Lint 沉淀 | 按需连接（notion 已连） |
| **云存储 / 文件** | **百度网盘（baidu-netdisk，已连）** | 用户自有文件、报告归档与投递 | ✅ 真实可用（已连） |
...
> **诚实声明（重要）**：本 Skill 只集成**当前环境真实存在**的 Skill / 连接器。Firecrawl、Crunchbase Pro、PitchBook 等在本环境未提供的服务**不会**被声明为已集成——若你所在平台提供，可自行在 Step 1 搜索入口追加，但 README / SKILL.md 不虚假标注。**GitHub MCP 已于 v2.2.1 确认真实连接**（`mcp__github__*` 工具齐全），可直连搜索/代码/Issue/PR/Release，亦可用 `gh` CLI 兜底。当前环境已安装/已连的互补 skill 与 MCP（方法论已被 dmr 吸收，缺失一律优雅降级）：`agent-reach`、`agent-browser`、`markitdown`、`tdx-connector`、`patsnap-search`（智慧芽）、`zhihu`、`ima-mcp`、`wechat-article-search`、`literature-search`（最优方法论参考）、`intel-osint-daily`、`macro-monitor`、`notion`（已连）、`agentkey`（已连，聚合数据 API）、`baidu-netdisk`（已连，云存储）；按需连接已注册：`pkulaw`（北大法宝，已连未启用）、`qixinhuiyan-mcp`（启信慧眼，按需）。**v2.2.1 已永久删除（不可逆）**的重复冗余 skill（方法论已并入 dmr）：`google-scholar-search`（实为 Semantic Scholar 封装）、`academic-research-hub`（Proprietary + OpenClawCLI）、`deep-research`（工作流已吸收）、`news-summary`（RSS 已吸收）、`perplexity` / `tavily`（与 WebSearch 重复的 AI 搜索）。标 🆓 的学术/数据源为免费公开 API，无需专用连接器即可直调。
...
- **完整更新史**：v2.0.0 → v2.2.8 每项变更细节 —— 见 SKILL.md [附录 A](SKILL.md#附录-a完整更新史v200--v228)。

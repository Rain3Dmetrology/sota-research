...
```mermaid
flowchart TB
  subgraph MAIN["主管线 · Step 0–8（确定性流水线）"]
    direction LR
    S0["0 范围收敛"] --> S1["1 多源采集"]
    S1 --> S2["2 去重 · 去旧"]
    S2 --> S3["3 源分级 T1–T4"]
    S3 --> S4["4 交叉验证 ≥2 源去假<br/>竞品参数 ≥3 源"]
    S4 --> S5["5 矛盾消解"]
    S5 --> S6["6 吸收热评 T4"]
    S6 --> S7["7 对抗审计"]
    S7 --> S8["8 结构化输出 + 100 分"]
  end
  subgraph CLOSE["三-B 深度研究闭环（平台无关 · 纯提示词）"]
    direction LR
    P1["Phase1 初始调研"] --> P2["Phase2 规划大纲"]
    P2 --> P3["Phase3 逐章深研 · 审稿"]
    P3 --> P4["Phase4 撰写框架"]
    P4 --> P5["Phase5 发布整合"]
  end
  CLOSE -. "章节级进度通报" .- S1
...
```mermaid
flowchart TB
  subgraph L0["默认层 · 零依赖零安装（质量基座）"]
    direction LR
    D1["LLM 内置 web_search / web_fetch"]
    D2["免费 REST API（OpenAlex / arXiv / Crossref 等）"]
  end
  subgraph L1["可选增强层 · 缺失即优雅降级"]
    direction LR
    E1["Exa · Firecrawl · Tavily · Brave（搜索）"]
    E2["Perplexity Sonar（AI 搜索）"]
    E3["GPT Researcher（本地/远程深度闭环）"]
    E4["ModelScope 魔塔（中文模型/数据集）"]
  end
  L1 -. "仅丰富素材 · 非质量前提" .- L0
  L0 --> OUT["源分级 + 交叉验证 + 置信标签<br/>可复现调研报告"]
```

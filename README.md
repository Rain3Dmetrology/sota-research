...
| **代码 / 项目** | GitHub 搜索 + Trending（`gh` CLI 已认证 + web） | 开源实现、技术栈、Star/PR 趋势（github MCP 当前未连，走 gh CLI） | ✅ 真实可用（gh CLI） |
...
> **诚实声明（重要）**：本 Skill 只集成**当前环境真实存在**的 Skill / 连接器。Firecrawl、Crunchbase Pro、PitchBook 等在本环境未提供的服务**不会**被声明为已集成——若你所在平台提供，可自行在 Step 1 搜索入口追加，但 README / SKILL.md 不虚假标注。Tavily、agent-reach、agent-browser、markitdown、tdx-connector、patsnap-search（智慧芽）、zhihu、ima-mcp、perplexity、wechat-article-search、academic-research-hub、literature-search、news-summary、intel-osint-daily、macro-monitor、deep-research 等为当前环境已安装/已连的 skill/MCP；GitHub 走 `gh` CLI（github MCP 当前未连）。标 🆓 的学术/数据源为免费公开 API，无需专用连接器即可直调。v2.2.0 对学术三件套做**优胜劣汰**：`literature-search` 为最优方法论参考、`academic-research-hub`（Proprietary + 依赖 OpenClawCLI）降为可选不硬依赖、`google-scholar-search`（实为 Semantic Scholar 封装、命名误导）列为**卸载候选**。dmr 均按需作为可选源/方法论引用，缺失一律优雅降级。

...
description: >
  ...
  quality: NATO-adapted 4-tier source grading, mandatory >=2-source cross-validation (>=3 for competitor key parameters),
...
compatibility: >
  ...
  AgentKey aggregated-data API as an optional fallback search/data entry.
  Optional CN-friendly AI search backends (key-gated, NON-default, gracefully skip if absent):
  AnySearch (pre-filter + RRF dedup; its 76.4% benchmark is a vendor self-claim → tag [VENDOR CLAIM],
  do NOT treat as independent proof); 秘塔搜索 (CN AI search with 事实检验 fact-check, pay-per-call, needs key).
  ...
  zhihu (search_content + hot_list), ima-mcp / notion (knowledge base),
  baidu-netdisk (cloud storage); agentkey available as an aggregated search/data fallback entry.
  On-demand MCPs (need user enable/connect): tyc-mcp / qcc-company / qixinhuiyan-mcp
  (enterprise registry/risk), sec-edgar (US SEC filings), pkulaw (北大法宝).
  GitHub MCP: mcp__github__* tools available — search_repositories, search_code,
...
2. **生成/验证分离**：综合（写）与核查（验）分阶段进行；普通事实单元须 ≥2 个独立源确认才标 `Confirmed`，**竞品关键参数（定价/版本/MCP/许可/收购归属/配额）须 ≥3 独立源**（见质量规则 18）。
...
- ✅ **Confirmed** — ≥2 个独立 Tier 1–3 源一致确认（**竞品关键参数须 ≥3 独立源**，见质量规则 18）
...
│  每点需 ≥2 独立源确认才标 Confirmed（竞品关键参数 ≥3，见规则 18）；│
...
## 三-B、工具路由与三层收敛兜底（质量优先 · 不替 WebSearch · 不降质量）

> 本节为 Step 1 采集的**工具分派总纲**，定义「选什么工具、按什么顺序兜底」。总优先级：**质量+全面性 > 免费 > 速度 > 0key**。0key 工具是**兜底不断流**层，不是质量首选。四去管道（去重/去旧/去假/去矛盾）保证质量，不依赖某个具体搜索 API——即使用 L2/L3 零 key 工具，输出质量也不崩。

**三层收敛兜底（每层工具收敛、三层即可，不是越多越好）：**

| 层 | 定位 | 工具（收敛选择） | 是否需 key | 解锁能力 |
|----|------|----------------|-----------|----------|
| **L1 主用（质量优先）** | 深度+广度+摄取的供给源 | **GPT Researcher**（深度闭环）+ **Exa / Tavily**（垂直/神经索引广度）+ **Firecrawl**（全站抓取/PDF·RFC/常驻监控） | 需（用户个性化提供） | 最高深度与广度；GPT Researcher 多步检索+综合，Exa/Tavily 垂直索引，Firecrawl 结构化摄取 |
| **L2 免费/零 key 兜底** | 注册/限流即可用，质量中上 | Exa keyless（`mcp.exa.ai/mcp`，20k/mo 免费）、Firecrawl Keyless（`mcp.firecrawl.dev/v2/mcp`，1k/mo 免费）、Brave 免费（2k/mo，独立索引交叉验证）、Perplexity 免费（Sonar 带引用） | 部分需免费 key、部分零 key | 无 L1 key 时仍可高质量广覆盖；Brave 作独立索引交叉验证 |
| **L3 终极零 key 兜底** | 永远不断流 | **DuckDuckGo MCP**（`@nickclyde/duckduckgo-mcp-server`，零 key 无限按限流）+ 内置 **WebSearch / web-access** | 零 key | 任何人都开箱即用；保证检索不断流 |

**路由与降级逻辑：**
1. 用户声明「质量优先 / 深度调研」→ 优先 L1；若 L1 所需 key 未提供 → 自动降级 L2，并诚实标注「深度闭环降级为广覆盖，质量上限受 L2 限制」。
2. L2 任一源限流/失败 → 切同层其他源或降级 L3；每源记 (url, 日期, 层级)。
3. L3 为不可绕过的最终兜底：DDG + 内置 WebSearch 保证检索永不空转。
4. 默认发布 L2/L3 零 key 配置（`references/tool-routing.md` 提供 `mcp.json`），**任何人复制即可跑，无需注册任何账号**；质量追求者按 §setup 个性化提供 L1 key。

**质量底座不变**：无论走 L1/L2/L3，Step 2–8（四去管道 + 源分级 + 交叉验证 + 矛盾消解 + 对抗审计）全程不变。工具只决定「从哪里取素材」，质量由流程决定。
...
| 步骤 | 用到的能力 | 备注 |
|------|-----------|------|
| 搜索入口 | `web-access` / `agent-reach`（web/search）+ `agentkey`（聚合数据 API，可选兜底）；内置 WebSearch/WebFetch 优先；Tavily/Perplexity 若有 key 可直调其 API（skill 已于 v2.2.1 卸载） | WebSearch 内置最稳、ToS 干净；agentkey 可替代缺失的专业 MCP |
| 社媒/热评 | `agent-reach` / `agent-browser`（小红书/知乎/Reddit/Bluesky/X/B站/YouTube）+ `web-access` | 复用登录态、抓评论；UGC 仅作 T4 信号 |
| 专利 | `patsnap-search` MCP；公开库通过 WebSearch/WebFetch | 技术壁垒分析 |
| 财经/财报 | `westock-mcp` / 通达信 `tdx-connector` MCP + `markitdown` 读 PDF | A 股/港股/美股基本面 |
| 企业工商/风险 | `tyc-mcp`（天眼查）/`qcc-company`（企查查）/`qixinhuiyan-mcp`（启信慧眼）MCP，按需连接 | 股权、司法、经营异常、行政处罚、企业风险洞察 |
| 美股 / SEC | `sec-edgar` MCP，按需连接 | 10-K/10-Q/附注 |
| 法律/合规 | `wk-workbuddy` / `yuandian-mcp` / `pkulaw`（北大法宝）MCP | 诉讼/资质核查/法律法规检索 |
| 代码/项目 | `github` MCP + `gh` CLI（已认证）+ web | 开源实现/技术栈/Star·PR 趋势（MCP 直连优先） |
...
| 知乎(技术+反馈) | `zhihu` MCP（search_content + hot_list） | 中文技术教程、用户反馈、产品体验交叉验证（T4 信号） |
| AI 搜索(可选) | 若有 `PERPLEXITY_API_KEY`/`Tavily`/AnySearch key 直调其 API（skill 已于 v2.2.1 卸载）；秘塔搜索（国内 AI 搜索，含事实检验，需 key）作 CN 场景可选增强；否则内置 WebSearch | 带引用 AI 搜索；均为可选、非默认、无 key 优雅跳过 |
...
| 知识库 | `ima-mcp` / `notion` / Obsidian / 本地 wiki | 用户自有资料 |
| 云存储 / 文件 | `baidu-netdisk` MCP | 用户文件、报告归档 |
...
18. ❌ 竞品关键参数仅 ≥2 源 → **竞品关键参数**（定价/版本号/MCP 支持情况/许可协议/收购归属/免费额度/配额等可量化事实）须 **≥3 独立源**（其中 ≥1 须 T1/T2 官方或权威机构）才标 `Confirmed`；普通事实维持 ≥2（控成本，避免普通事实成本爆炸）。仅 1–2 源的竞品关键参数降 `Single-source` 并标 `待更多源确认`，**禁止据 ≤2 源下竞品关键结论**（见 三-B 节与 references/tool-routing.md）。

19. ❌ 把 0key 当质量首选 → 0key 工具（DDG MCP / keyless Exa·Firecrawl / 内置 WebSearch）是**兜底不断流**层（L2/L3），非质量首选；质量优先主用层（L1：GPT Researcher + Exa/Tavily + Firecrawl，需 key）由用户个性化提供。任何人零 key 可跑 L2/L3，但深度/广度质量上限由 L1 决定。技能默认发 L2/L3 零 key 配置，质量追求者按 references/tool-routing.md 的 §setup 个性化开 L1（见 三-B 节）。
...
**Q4：需要付费 API key 吗？**
A：**默认不需要，零 key 即可跑。** 技能默认发布 L2/L3 零 key 配置（DDG MCP + keyless Exa/Firecrawl + 内置 WebSearch，见 references/tool-routing.md），任何人复制 `mcp.json` 开箱即用，无需注册任何账号。若追求**质量优先深度调研**，可个性化提供 L1 key（OpenAI + Exa/Tavily + Firecrawl，见 三-B 节）；未提供则自动降级 L2/L3 并诚实标注质量上限。学术源（OpenAlex/Semantic Scholar/Crossref/arXiv/PubMed 等）始终 🆓 免费 API 直调，无需 key。
...
- **v2.3.0**：工具路由分层化（质量优先）。新增「三-B、工具路由与三层收敛兜底」节——明确总优先级 **质量+全面性 > 免费 > 速度 > 0key**，定义 **L1 主用**（GPT Researcher 深度闭环 + Exa/Tavily 广度 + Firecrawl 摄取监控，需 key）/ **L2 免费零 key 兜底**（Exa keyless / Firecrawl Keyless / Brave 免费 / Perplexity 免费）/ **L3 终极零 key 兜底**（DuckDuckGo MCP + 内置 WebSearch）；竞品关键参数交叉验证由 ≥2 升 **≥3**（普通事实维持 ≥2 控成本）；新增 `references/tool-routing.md`（零 key 默认 `mcp.json` + L1 keyed 升级片段 + §setup 个性化流程）；质量规则增 18（竞品关键参数 ≥3）、19（0key 兜底非首选）；frontmatter 描述与 〇/二/Step4/FAQ Q4 同步对齐。本地 / 仓库 / 发布包三端统一为 v2.3.0。
